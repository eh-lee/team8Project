...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    // console.log("네임과 룸 그리고 maxParty를 알려주마" ,name, room, maxParty);
    setRoom(room);
    setName(name);
    ...
    socket.on('sessionId', ({ sessionId, text}) => {
      console.log('소켓아이디다:', sessionId, text);
    ...
    socket.on("currParty", ({ numUsers, maxParty, room }) => {
      ...
      console.log("currParty가 들어오는가?", currParty);
    });
    // ================ socket server와 통신하기 ==================
    ...
    socket.on("message", ({user, text}) => {
      console.log('message on한 거. 어떻게 들어오나', user, text);
      setMessages((messages) => [...messages, message]);
    });   
    console.log("message이벤트 on해서 messages에 들어간 거" ,messages)
    ...
    return () => {
      socket.disconnect();
      console.log('소켓 닫아유~ disconnect');
      };

  // }, [ENDPOINT, window.location.search]);
  }, []);

  // console.log("name, room, maxParty는 set 된 거야??", name, room, maxParty);
  ...
  // const connectSocket = () => {
  //   // const savedSessionId = sessionStorage.getItem('socketSessionId');
  //   // const connectionOptions = savedSessionId ? { query: { sessionId: savedSessionId } } : {};
